 ...
 * allocate.c obviously based on the "arenas" or "bump-allocation".
 *
 ...
 * and lots of RAM though a little bit of arena usage can possibly
 * make these better where it really matters.
 *
 ...
 * anyone wants to give it a try). Like most of c-armyknife-lib, the
 * emphasis is therefore on correctness and simplicity over
 ...
 * which was actually my initial reason for not really worrying about
 * either allocation or deallocation in allocate.c. Though allocate.c
 * was initially only meant to be a thin shim on top of C's
 * malloc/free that would simply allow turning the switch to the Boehm
 * collector (which I think would still be interesting to "provide" at
 * some point), it evolved to have some interesting debugging options
 * (over-run detection) which may be kept here.
 *
